...
public class Compiler2BlobHost implements Blob {
	...
	public Compiler2BlobHost(ImmutableSet<Worker<?, ?>> workers,
		...
		this.coreCode = runnableProxies(coreCodeHandles.build());
	...
	private static ImmutableList<Runnable> runnableProxies(List<MethodHandle> handles) {
		try {
			Module m = new Module();
			...
			for (int i = 0; i < handles.size(); ++i) {
				Klass proxyKlass = new Klass("Proxy"+i, m.getKlass(Object.class), ImmutableList.of(m.getKlass(Runnable.class)), m);
