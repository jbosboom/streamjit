...
public final class Scheduler {
	...
	public static <T> ImmutableMap<T, Integer> schedule(List<Channel<T>> channels) {
		...
		Vector<Rational> vector = new Vector<>(Collections.nCopies(channels.size(), Rational.FACTORY.get(0)).toArray(new Rational[0]));
		...
		if (v.getEntry(1).compareTo(Rational.FACTORY.zero()) < 0)
			v.multiplyReplace(Rational.FACTORY.get(-1));
	...
	public static void main(String[] args) {
		...
		Channel<Object> channel2 = new Channel<>(a, c, 5, 7);
